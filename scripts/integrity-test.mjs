/*
 * Backend fidelity guard: run a heavy churn workload against real FASTFFS over
 * the emulated NOR device and assert every file reads back byte-exact — through
 * garbage collection and sector reuse. If the device's NOR emulation (the AND-
 * only program, sector erase) ever drifts from the reference, this catches it.
 */
...
for (let op = 0; op < 3000; op++) {
  ...
  } catch { /* NO_SPACE etc. — FASTFFS keeps the last committed version; truth unchanged */ }
}
